...
class getCookie():
    ...
    def run(self, pcname, queue_url, adsIDlist):
        ...
        self.usego.sendlog(f"{queue_url} 执行")
            ...
            for cookiedata in batch_response_cookie:
                ...
                try:
                    ...
                    if "sql 语句异常" in str(sql_data):
                        ...
                        self.usego.sendlog(f'{cookiedict["user_id"]},入库成功！！！')
                except Exception as e:
                    self.usego.sendlog(f" 出现异常了: {e}")
                    batch_failed_data.append(cookiedict)  # 失败的数据保留
        ...
        if failed_data:
            self.usego.sendlog(f"以下数据入库失败：{failed_data}")
            for cookiedata in failed_data:
                ...
                try:
                    sql_data = self.ssql.note_users_info_update_cookie(useragent=cookiedict["useragent"], cookie=cookiedict["cookie"],
                                                                       adsID=cookiedict["user_id"])
                    if "sql 语句异常" in str(sql_data):
                        self.usego.sendlog(f"没救了，二次入库失败{cookiedict}")
                    else:
                        self.usego.sendlog(f'{cookiedict["user_id"]},入库成功！！！')
                except Exception as e:
                    self.usego.sendlog(f"没救了，二次入库失败{cookiedict}")

        self.usego.sendlog("所有批次的 get_cookie 执行完毕，可以执行后续操作")
    ...
    def get_cookie(self, adsID):
        ...
        # 打印 Node.js 脚本的标准输出
        self.usego.sendlog(f"Node.js 脚本的标准输出:{result.stdout}")


        # 打印 Node.js 脚本的标准错误
        if result.stderr:
            self.usego.sendlog("Node.js 脚本的标准错误:")
            self.usego.sendlog(result.stderr)

        # 检查 Node.js 脚本是否执行成功
        if result.returncode == 0:
            self.usego.sendlog("Node.js 脚本执行成功")
            return result.stdout
        else:
            self.usego.sendlog(f"Node.js 脚本执行失败，错误码: {result.returncode}")
            return None
